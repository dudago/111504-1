...
public class use_QRCode : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        if (Application.HasUserAuthorization(UserAuthorization.WebCam))
        {
            ...
            InvokeRepeating("DecodeQR", 0, 0.1f); // 0.5 秒掃描一次
        }
    }
    ...
    public void click_start(){
        if(txtQRcode.text  == "use"){
            // && txtQRcode.Substring(3,5) == "01";
            webCameraTexture.Stop();
            SceneManager.LoadScene("coupon");
            // webCameraTexture.Stop();
        }
        else if(txtQRcode.text  == "get"){
            webCameraTexture.Stop();
