...
public class explorehot : MonoBehaviour
{
  int count;
    int over;
    ...
    void Start()
    {
        ...
        DataSet ds = sql.QuerySet("SELECT a.name,b.url FROM `111- qingshui`.attractions as a join `111- qingshui`.images as b on a.idattractions=b.attractionid where categoryID=3 limit 5;");
            if (ds != null)
            {
                count=0;
